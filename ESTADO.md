...
## Sesion 15 — privacidad, soporte y recuperación (2026-08-11, en curso)
- Se implementó una puerta de consentimiento de servicio versionada (`2026-08-11`) después de verificar identidad. No usa `localStorage` como evidencia legal: registra propósito, versión, idioma, origen y fecha en `data_consents`.
- Cuenta y privacidad quedó disponible desde el avatar de `/app`: descarga de datos, gestión de suscripción en Hotmart, cierre de sesión, textos legales y eliminación de cuenta.
- Exportación protegida: toma el usuario exclusivamente de la sesión, reúne datos del servidor y del dispositivo y registra auditoría con hash SHA-256 sin correo ni UUID.
- Eliminación protegida: exige sesión iniciada en los últimos 15 minutos, frase exacta, confirmación explícita y protege la cuenta propietaria. Elimina Auth/datos por cascada, limpia registros auxiliares y conserva solo auditoría no identificable. Aclara que borrar la app no cancela Hotmart.
- Soporte privado añadido dentro de la cuenta con categorías cerradas, límite de 1.000 caracteres, mismo origen, sesión obligatoria y control de frecuencia. El panel admin muestra las solicitudes, avisa si hay clientes esperando y permite marcarlas como resueltas; SLA operativo propuesto: menos de 24 horas hábiles.
- Migración `20260811210000_privacy_operations.sql` aplicada por la propietaria en Supabase el 2026-08-13; el editor confirmó la ejecución. Crea consentimiento, auditoría y tickets con RLS. La verificación funcional se cerrará después del despliegue.
- Backups: se documentó respaldo lógico, almacenamiento cifrado y restauración en proyecto de prueba. Supabase Free no aporta el respaldo administrado requerido; ejecutar y cronometrar el restore sigue bloqueando una certificación plena. Supabase Pro se recomienda al primer ingreso recurrente, sin contratarlo todavía.
- Revisión legal: textos ampliados para minimización, terceros, retención, 18+, exportación y borrado. La revisión de un profesional aplicable a los países de venta permanece obligatoria antes de tráfico abierto.
- Evidencia local: TypeScript y ESLint pasan; 19/19 pruebas (Hotmart 10, personalización 6, privacidad 3) pasan. La repetición del build quedó bloqueada únicamente porque el entorno de auditoría no pudo descargar Google Fonts; el último build completo anterior compila 16 rutas y debe repetirse en Vercel sobre el SHA final.
- Seguridad estática: ninguna clave privada está versionada; solo `.env.example`. No se detectaron CORS abierto, logs de depuración, criptografía débil ni IDs de usuario aceptados por los endpoints de privacidad. La repetición de `pnpm audit` quedó sin resultado por bloqueo de red del entorno; la auditoría anterior sobre las mismas dependencias informó cero vulnerabilidades.
- QA móvil de Cuenta y privacidad ya fue aprobada por revisor independiente: 39/40 usabilidad, 18/20 craft, sin desbordamiento y con acción destructiva inequívoca. La captura adicional del formulario de soporte no pudo repetirse porque el navegador de auditoría bloqueó localhost; queda pendiente junto con el despliegue.
- Publicación de la capa completada: commit `2f9350d` enviado a `main`; Vercel desplegó `2KVcLh6LWkdwWBnZeq8qM8k7qmpy` con estado `Ready`, dominio `menu.centrodigitalglobal.online` y fuente exacta `2f9350d`.
- Verificación pública posterior: la landing responde con su contenido completo; `/app` sin sesión redirige a `/login` y `/admin` sin sesión redirige a `/login?next=%2Fadmin`. La navegación directa al endpoint privado fue bloqueada por el cliente de auditoría, por lo que consentimiento, exportación y soporte requieren una última prueba autenticada de la propietaria.
- Prueba autenticada de soporte en celular: la primera solicitud fue recibida y el segundo envío fue rechazado por el límite anti-duplicados, confirmando persistencia y control de frecuencia.
- Hallazgo de acceso admin: el enlace se solicitó en computadora y se abrió en el celular, por lo que el verificador PKCE ligado al navegador no estaba disponible y la app lo rechazó como inválido. Se corrigió además la pérdida de `next`: destinos cerrados `/app` y `/admin` se preservan en el correo y callback, se bloquean redirecciones externas y el acceso admin vuelve a la sección exacta. Evidencia local: TypeScript, ESLint y 5/5 pruebas de destino/privacidad pasan.
- Acceso admin E2E confirmado después de publicar `69df887`: el enlace solicitado y abierto en la misma computadora llevó directamente a `/admin?section=errors`. La solicitud privada de prueba apareció con aviso automático y fue marcada como resuelta; el contador bajó de 1 a 0.
- La captura del panel mostró 10 registros históricos de `personalize_week_failed:42P01`/`personalization_failed`, originados antes de aplicar la migración de personalización. Se añadió cierre trazable por grupo desde el panel: actualiza `status=resolved` y `resolved_at`, no borra historial ni oculta fallos futuros. También se corrigió el singular `1 solicitud`. TypeScript y ESLint pasan.
